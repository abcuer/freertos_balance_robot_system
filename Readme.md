...
- **CtrlTask (最高优先级)** 负责 **MPU6050 角度解算**及**直立+速度+转向 PID**运算，确保控制周期达 **5ms**。
...
| 模式名称 | 功能描述 | 核心量化指标 |
| :--- | :--- | :--- |
| **平衡模式** | 拥有稳健的自平衡能力，抗干扰鲁棒性强。 | 直立环采样周期 **5ms**，平衡倾角波动 **< 2°**。 |
| **蓝牙遥控模式** | 通过 HC-06 模块与手机 App 连接，实现**任意方向遥控**。 | 遥控响应延迟 **< 10ms**；具备 **7cm** 障碍物自动警报功能。 |
| **超声波跟随** | 小车自动保持与前方目标的距离。 | 跟随距离维持在约 **25cm**；测距精度达 **±2cm**。 |
## 3. 传感器技术
### 蓝牙数据异步解析
蓝牙解析采用**信号量 + 串口中断**的组合方案：
- **实时性**：串口中断触发信号量，仅在有数据时唤醒解析任务，实现按需计算。
- **低CPU占用**：任务挂起时不占用时间片，保证测距与电机 PID 全速运行。
- **任务解耦**：中断仅发信号，执行时间极短，避免阻塞测距或定时器捕获。
...
- **着陆检测 (Detect_PutDown)**：在锁定状态下，若满足“角度回正 (<10°)”、“角速度平静”且“轮子静止”持续 20 个周期 (约 0.75s) 时，自动重置 PID 积分并恢复平衡。
## 📺 演示与展示
- ### 资源占比情况
